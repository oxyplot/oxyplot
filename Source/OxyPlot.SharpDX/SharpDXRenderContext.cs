...
namespace OxyPlot.SharpDX
{
    public class SharpDXRenderContext : IRenderContext, IDisposable
    {
        ...
        public void DrawRectangle(OxyRect rect, OxyColor fill, OxyColor stroke, double thickness)
        {
            ...
            if (fill.IsVisible())
            {
                renderTarget.DrawRectangle(r, GetBrush(fill));
            }
        }
        ...
        {
            var group = new GeometryGroup(this.d2dFactory, FillMode.Alternate,
              rectangles.Select(x => new RectangleGeometry(this.d2dFactory, x.ToRectangleF())).ToArray());
